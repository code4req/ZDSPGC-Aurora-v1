...
const BentoCard = ({ event, onSelect }) => {
  ...
  return (
    ...
    >
      <div className={`absolute -right-10 -top-10 h-48 w-48 rounded-full bg-gradient-to-br ${event.color} opacity-20 blur-3xl group-hover:opacity-40 transition-opacity duration-500`} />
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="text-xs font-mono text-emerald-500/80 uppercase tracking-widest">{event.attendees} ENROLLED</span>
        </div>
...
const Events = () => {
  ...
  const categories = [
    ...
    { id: 'sports', label: 'Sports', icon: FaBasketballBall },
    { id: 'cultural', label: 'Cultural', icon: FaTrophy },
    { id: 'workshop', label: 'Workshops', icon: FaMicrophone },
  ...
  const featuredEvents = [
    {
      ...
      location: "ZDSPGC Main Grounds",
      category: "cultural",
      icon: FaTrophy,
      description: "Join us for a day of celebration with performances, games, and community activities.",
    ...
    },
    {
      ...
      category: "workshop",
      icon: FaFlask,
      description: "Explore the latest in technology and innovation with industry experts and hands-on workshops.",
    ...
    },
    {
      ...
      description: "Cheer for our athletes as they compete against other schools in various sports events.",
  ...
  const allEvents = [
    ...
    },
    {
      ...
      location: "ZDSPGC Grand Hall",
      category: "academic",
      icon: FaGraduationCap,
      description: "Celebrate the achievements of our graduating students as they embark on their next journey.",
    ...
    },
    {
      ...
      date: "July 8, 2026",
      time: "8:00 AM - 5:00 PM",
      location: "ZDSPGC Library",
      category: "academic",
      icon: FaBook,
      description: "Present your research and learn from fellow students and faculty members.",
    ...
    },
    {
      ...
      location: "Aurora Community",
      category: "community",
      icon: FaHandshake,
      description: "Give back to the community through various service activities and social programs.",
  ...
  useGSAP(() => {
    ...
    const image = document.querySelector(".zentry-mask-clip img");
    if (image) {
      gsap.fromTo(image, 
        {
          scale: 2,
          rotation: 5, // counter-rotate image so it feels dynamically floating
        },
        {
          scale: 1,
          rotation: 0,
  ...
  return (
    <section className="relative w-full min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-emerald-950 via-zinc-950 to-black text-white overflow-hidden selection:bg-emerald-400 selection:text-black">
      ...
      <div className="h-dvh w-full relative mt-12 overflow-hidden flex items-center justify-center" id="clip">
        <div 
        ...
        >
          ...
          <div className="absolute bottom-12 left-1/2 transform -translate-x-1/2 z-10 text-white text-center w-full px-4">
            <h3 className="text-3xl md:text-6xl font-black uppercase tracking-wide drop-shadow-2xl mb-2">
              Experience <span className="text-emerald-400">Excellence</span>
            </h3>
      ...
      {/* FEATURED CAROUSEL */}
      <div ref={featuredRef} className="relative py-24 px-6 max-w-7xl mx-auto">
        <div className="flex items-end justify-between mb-12">
          <div>
            <p className="text-xs font-mono uppercase tracking-widest text-emerald-400 mb-2">Spotlight</p>
            <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tight text-emerald-50">Featured Realm</h2>
          </div>

          <div className="flex items-center gap-3">
            <button 
              onClick={() => setCurrentSlide((prev) => (prev - 1 + featuredEvents.length) % featuredEvents.length)}
              className="h-12 w-12 rounded-full border border-emerald-500/30 bg-emerald-950/80 flex items-center justify-center text-emerald-200 hover:bg-emerald-400 hover:text-black transition-all"
            >
        ...
        <div className="relative overflow-hidden rounded-3xl border border-emerald-500/20 bg-emerald-950/40 p-8 md:p-14 backdrop-blur-md">
          {featuredEvents.map((event, index) => {
            ...
            const Icon = event.icon;
            return (
              <div key={event.id} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                <div className="lg:col-span-7 space-y-6">
                  ...
                  <div className="flex flex-wrap gap-6 pt-4 text-xs font-mono text-emerald-300">
                    <span className="flex items-center gap-2"><FaCalendarAlt className="text-emerald-400" /> {event.date}</span>
                    <span className="flex items-center gap-2"><FaClock className="text-emerald-400" /> {event.time}</span>
                    <span className="flex items-center gap-2"><FaMapMarkerAlt className="text-emerald-400" /> {event.location}</span>
                  </div>
                </div>
                ...
                <div className="lg:col-span-5 flex justify-center">
                  <div className="relative w-full aspect-square max-w-sm rounded-2xl border border-emerald-500/30 bg-gradient-to-br from-emerald-950 to-zinc-950 p-8 flex flex-col items-center justify-center text-center group shadow-2xl">
                    <div className={`p-6 rounded-full bg-gradient-to-br ${event.color} text-white mb-6 shadow-2xl group-hover:scale-110 transition-transform`}>
                      <Icon className="text-5xl" />
                    </div>
                    <span className="text-2xl font-black uppercase tracking-wider text-emerald-100">{event.attendees}</span>
                    <span className="text-xs font-mono text-emerald-500 uppercase mt-1">Expected Attendees</span>
                  </div>
                </div>
              </div>
            );
      ...
      {/* BENTO EVENTS GRID */}
      <div ref={eventsRef} className="py-24 px-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredEvents.map((event) => (
            <BentoCard key={event.id} event={event} onSelect={setSelectedEvent} />
          ))}
        </div>
      </div>

      {/* CTA SECTION */}
      <div ref={ctaRef} className="relative py-32 px-6 border-t border-emerald-500/20 bg-black text-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto space-y-8">
          <p className="text-xs font-mono uppercase tracking-widest text-emerald-400">Enter The Campus Nexus</p>
          <h2 className="text-5xl md:text-7xl font-black uppercase tracking-tight leading-none text-emerald-50">Ready To Participate?</h2>
